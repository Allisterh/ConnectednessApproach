...
DATA = read.xlsx("./GabauerGupta2019.xlsx", detectDates=TRUE)
DATA = na.omit(DATA)
DATE = as.Date(DATA[,1])
RAW = DATA[,-1]
NAMES = colnames(RAW)
...
plot(DATE, RAW[,1], type="l", ylim=c(min(RAW), max(RAW)), las=1, xlab="", ylab="", xaxs="i", yaxs="i")
grid(NA, NULL)
abline(h=0, lty=3)
for (i in 1:k) {
...
Y = matrix(NA, nrow=t-1, ncol=k)
for (i in 1:k) {
  Y[,i] = diff(RAW[,i])/sd(diff(RAW[,i]))
}
date = DATE[-1]
...
for (i in 1:k) {
  plot(date, Y[,i], type="l", las=1, xlab="", ylab="", xaxs="i", yaxs="i", main=NAMES[i], col="steelblue4",ylim=c(min(Y),max(Y)))
  grid(NA,NULL)
